...
// private[annotator] class LinnaeusNERAdapter (protected val dal : DAL = current.dal) extends NERAdapter {
  ...
  // override protected def annotate(document : Document) : Future[FinishedAnnotation] =
    // obtainMentions(document.text) map {
      // mentions => mentions foreach { saveMention(_, document) }
